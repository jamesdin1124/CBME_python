...
MEDICAL_PROMPT = (
    "這是一位主治醫師在教學醫院指導實習醫學生（clerk）或住院醫師（resident）"
    "照護病人後，對其臨床表現給予口頭回饋評語。"
    "內容通常包括：病史詢問、身體檢查、鑑別診斷、處置計畫、病歷書寫、"
    "醫病溝通、臨床決策、緊急處置等面向的評價與建議。"
    "語言以繁體中文為主，夾雜英文醫學術語。\n"
    "常見詞彙：EPA, clerk, 實習醫學生, 住院醫師, R1, R2, R3, 主治醫師, VS, "
...
# GPT 系統提示：潤飾醫學教學回饋
GPT_SYSTEM_PROMPT = (
    "你是醫學教育領域的文字編輯助手，專門整理主治醫師對實習醫學生（clerk）"
    "或住院醫師（resident）在照護病人時的臨床表現口頭回饋。"
    "情境是教學醫院中，老師觀察學員接診、查房、值班、處置後給予的評語。\n"
    "請依照以下規則處理：\n"
