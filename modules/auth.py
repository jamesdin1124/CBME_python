...
PERMISSIONS = {
    'admin': {
        ...
        'can_approve_all_departments': True,  # 可以審核所有科別申請
        'can_manage_department_admins': True  # 可以管理科別管理員
    },
    'department_admin': {
        'can_view_all': False,          # 科別管理員不能看到全部資料
        'can_edit_all': False,
        'can_manage_users': False,
        'can_upload_files': True,
        'can_view_analytics': True,
        'can_view_department_data': True,   # 可以看到自己科的資料
        'can_view_ugy_data': True,          # 可以看到UGY資料
        'can_view_pgy_data': True,          # 可以看到PGY資料
        'can_view_resident_data': True,     # 可以看到自己科的住院醫師資料
        'can_manage_department_users': True, # 可以管理本科使用者
        'can_approve_department_applications': True  # 可以審核本科申請
    },
    'teacher': {
    ...
    'resident': {
        ...
        'can_view_resident_data': True,     # ✅ 改為 True：允許進入住院醫師頁面（內容會被過濾為只看自己）
        'can_view_own_data': True          # 只能看到自己的資料
    },
    'pgy': {
        'can_view_all': False,          # PGY不能看到全部資料
        'can_edit_all': False,
        'can_manage_users': False,
        'can_upload_files': False,
        'can_view_analytics': False,
        'can_view_department_data': False,  # 不能看到科別資料
        'can_view_ugy_data': False,         # 不能看到UGY資料
        'can_view_pgy_data': False,         # 不能看到其他PGY資料
...
def authenticate_user(username, password):
    ...
    Returns:
        dict: 包含使用者資訊的字典，包括 role, name, department 等
        None: 驗證失敗
    """
    try:
        ...
        # 從 Supabase 查詢使用者
        result = conn.client.table('pediatric_users').select('*').eq('username', username).eq('is_active', True).execute()
...
def create_user(username, password, role, name, student_id=None, department=None, extension=None, email=None):
    """
    建立新使用者（純 Supabase 版本）

    Args:
        username: 使用者名稱
        password: 密碼 hash 或明文（函數會檢查並處理）
        role: 角色
        name: 姓名
        student_id: 學號（選填）
        department: 科別（選填）
    ...
    try:
        ...
        # 檢查使用者是否已存在
        existing = conn.client.table('pediatric_users').select('username').eq('username', username).execute()
        if existing.data and len(existing.data) > 0:
            return False, "使用者名稱已存在"

        # 確保密碼是 hash
        if len(password) != 64:  # SHA-256 hash 長度為 64
            password_hash = hash_password(password)
        else:
...
def get_user_department(username):
    """
    獲取使用者的科別（純 Supabase 版本）

    Args:
        username: 使用者名稱

    Returns:
        str: 科別名稱，如果沒有則返回 None
    """
    try:
        from modules.supabase_connection import SupabaseConnection
        conn = SupabaseConnection()

        result = conn.client.table('pediatric_users').select('department').eq('username', username).eq('is_active', True).execute()
