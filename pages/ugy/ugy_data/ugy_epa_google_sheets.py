...
def display_student_data(student_df, student_id, standard_categories=None):
    ...
    if proceeded_EPA_df is None:
        if 'processed_df' in st.session_state:
            proceeded_EPA_df = st.session_state['processed_df']
        else:
            st.warning("沒有可用的完整資料進行階層對比，將只顯示學生個人資料")
    ...
    with col1:
        ...
        # 使用新的個別學生雷達圖功能，直接顯示該同學及所有階層
        radar_fig = create_individual_student_radar_with_layers(
            student_df=student_df,
            student_id=student_id,
            all_data_df=proceeded_EPA_df,
