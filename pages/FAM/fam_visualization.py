        ...
        try:
            ...
            for i, student in enumerate(students):
                ...
                if not student_data.empty:
                    ...
                    fig.add_trace(go.Scatter(
                        x=student_data['年月'],
                        y=student_data['平均EPA分數'],
                        mode='lines+markers+text',
                        name=student,
                        line=dict(color=color, width=2),
                        marker=dict(size=8, color=color),
                        text=[f"n={int(count)}" for count in student_data['記錄數']],
                        textposition="top center",
                        ...
                        hovertemplate=f'<b>{student}</b><br>' +
                                     ...
                                     '平均EPA分數: %{y:.2f}<br>' +
                                     '記錄數: %{customdata[0]}<br>' +
                                     '標準差: %{customdata[1]:.2f}<br>' +
                                     '<extra></extra>',
                        customdata=list(zip(student_data['記錄數'], student_data['標準差']))
                    ))
            
            # 更新布局
            fig.update_layout(
                title="各住院醫師每月平均EPA分數趨勢圖",
                xaxis_title="月份",
                yaxis_title="平均EPA分數",
                yaxis=dict(range=[0, 5.2]),
                height=500,
