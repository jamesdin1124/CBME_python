...
def show_individual_analysis():
    ...
    st.markdown("### 技能分類進度")
    with st.expander("📖 可信賴程度分數說明"):
        st.markdown("""
| 分數 | 等級 | 說明 |
|:---:|:---|:---|
| **5** | 可以獨立執行並指導他人 | 可放手讓學員獨立操作，並能教導他人 |
| **4** | 可以獨立執行 | 可放手讓學員獨立操作，不需在場監督 |
| **3** | 可在間接指導下執行 | 操作者可執行，指導者不需在場但需可連絡 |
| **2** | 可在直接指導下執行 | 操作者可執行，但指導者需在場逐步指導 |
| **1** | 僅能觀摩 | 學員僅能在旁觀察，尚無法操作 |
""")
